...
void SevSeg::segmentOn(uint8_t segmentNum) {
  uint8_t pin = segmentPins[segmentNum];
  if (segmentOnVal == HIGH) {
    if (pin <= 7) PORTD |= (1 << pin); // PD0–PD7
    else PORTB |= (1 << (pin - 8));    // PB0–PB5
  } else {
  ...
  for (uint8_t digitNum = 0; digitNum < numDigits; digitNum++) {
    if (digitCodes[digitNum] & (1 << segmentNum)) {
      uint8_t digPin = digitPins[digitNum];
      if (digitOnVal == HIGH) {
        if (digPin <= 7) PORTD |= (1 << digPin);
        else PORTB |= (1 << (digPin - 8));
      } else {
        if (digPin <= 7) PORTD &= ~(1 << digPin);
        else PORTB &= ~(1 << (digPin - 8));
      }
...
void SevSeg::blank(void) {
  ...
  for (uint8_t dig = 0; dig < numDigits; dig++) {
    digitOff(dig);
  }
}
